...
const serverData = dataEl
  ? JSON.parse(dataEl.getAttribute("data-settings"))
  : {};
...
NonBioBTN.onclick = function () {
  sendServoRequest(
    TRASH_BINS["Non-Biodegradable"].servo_url,
    "nonbiodegradable"
  );
};
...
async function fetchu1() {
  try {
    ...
    if (data && distance > 0) {
      const save_data = await (
        await fetch("/send-sensor-level", {
          method: "POST",
...
//           NonBioBTN.click();
//         } else if (bestPrediction.className === "background") {
...
//           headers: { "Content-Type": "application/json" },
//           body: JSON.stringify({
//             name: bestPrediction.className,
//             confidence: predictionval,
//           }),
//         });
...
let isPredicting = false;
let highConfidenceStart = null;
let bestPredictionDuringPeriod = null;
...
async function predict() {
  ...
  if (!bestPredictionDuringPeriod || bestPrediction.probability > bestPredictionDuringPeriod.probability) {
    bestPredictionDuringPeriod = bestPrediction;
  }

  if (bestPrediction.probability >= predictionval) {
    if (!highConfidenceStart) {
      highConfidenceStart = Date.now(); // start the period
      console.log("⏱ High confidence period started...");
    } else {
      const elapsed = Date.now() - highConfidenceStart;
      if (elapsed >= period) {
        console.log(`Best class in ${period / 1000}s: ${bestPredictionDuringPeriod.className} (${(bestPredictionDuringPeriod.probability * 100).toFixed(2)}%)`);

        await fetch("/send-waste-detected", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ name: bestPredictionDuringPeriod.className, confidence: bestPredictionDuringPeriod.probability }),
        });

        switch (bestPredictionDuringPeriod.className) {
          case "recyclable":
            RecyBTN.click();
            break;
          case "biodegradable":
        ...
        highConfidenceStart = null;
        bestPredictionDuringPeriod = null;
        setTimeout(() => {
          isPredicting = false;
          console.log("Ready for next prediction...");
        }, 500);
    ...
    if (highConfidenceStart) {
      console.log("⚠️ Confidence dropped, resetting timer.");
      highConfidenceStart = null;
      bestPredictionDuringPeriod = null;
    }
  }
